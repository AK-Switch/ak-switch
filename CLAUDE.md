...
### 前置检查清单（强制，2026-07-05 新增）

声明任何任务"完成"之前，必须逐条核查以下清单。**跳过任意一条 = 任务未完成。**

1. **[测试]** — 如果有新增 CLI 命令或标志，是否写了对应的 CLI 入口测试？
2. **[测试]** — 是否运行了 `go test ./...` 并确认全量通过？
3. **[手动验收]** — 是否用真实二进制（`go install` 后）验证了改动行为？
4. **[提交]** — 是否在正确的分支上？提交信息是否清晰？
5. **[CI]** — 是否在前台等到 CI 绿？
