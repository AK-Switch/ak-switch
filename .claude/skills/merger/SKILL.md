...
## 工作流\r\n\r\n以下 7 个阶段构成完整的编排流程。每个阶段结束后，回看全局再进入下一阶段。\r\n\r\n> **工具选择：** 各阶段以 `gh` CLI 命令为主（功能完整、管道编排方便）。GitHub MCP 工具（`mcp__github__*`）可替代其中部分操作（如 `pull_request_read`、`list_pull_requests` 等），但 MCP 工具不支持管道编排和 JSON 过滤，在需要多步骤串联时不如 CLI 直接。遇到 MCP 可用时优先使用，但不必强求——用 CLI 完成全流程也是符合规范的。\r\n\r\n### 第一阶段：盘点
...
| `TestMetricsVerification_*` 失败 | flaky test（timing 敏感） | 见"常见陷阱 3"的鲁棒处理流程 |
...
# 批量开启（无依赖的 PR）
foreach ($pr in @(1,2,3)) { gh pr merge $pr --squash --auto --subject "merge: PR #$pr" }
```
...
### 当前 main 分支新增内容
- 从已合并 PR 的 title 自动生成（每个 PR 对应一条）
